...
function updateUI() {
    ...
    seats.forEach(seat => {
        ...
        seatElement.classList.remove('seat-available', 'seat-selected', 'seat-booked');
    ...
    return sortedRows.map(row => {
        ...
        return `
            <div class="seat-row">
                ...
                ${rowSeats.map(seat => {
                    const statusClass = seat.status === SEAT_STATUS.BOOKED 
                        ? 'seat-booked' 
                        : 'seat-available';
...
async function loadSeats() {
    ...
    try {
        ...
        // Fetch seats
        const seatsData = await API.get(`${API_CONFIG.ENDPOINTS.SEATS}?show_id=${showId}`);
...
async function handleBooking() {
    ...
    if (!confirm(`Bạn có chắc muốn đặt ${selectedSeats.length} ghế: ${selectedSeats.map(s => s.name).join(', ')}?`)) {
        return;
    }
    ...
    try {
        ...
        // Call booking API
        await API.post(API_CONFIG.ENDPOINTS.BOOK, {
            seats: seatIds
