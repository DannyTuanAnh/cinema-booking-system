...
function createMovieInfo(show) {
    return `
        <div class="movie-info-content">
            <img 
                src="https://via.placeholder.com/200x280?text=${encodeURIComponent(show.movie_title)}" 
...
function createShowtimesList(showtimes) {
    ...
    return Object.entries(groups).map(([date, shows]) => `
...
async function loadShowtimes() {
    ...
    try {
        ...
        // Fetch showtimes
        const showtimes = await API.get(`${API_CONFIG.ENDPOINTS.SHOWS}?movie_id=${movieId}`);
